...
def configure_host_firewall_for_webrtc():
    ...
    ufw_commands = [
        ...
        "sudo ufw allow in on lo",             # <--- ADD THIS LINE (Allows FastAPI/Browser local comms)
...
def launch_browser_dashboard(video_w=1920, video_h=1080, control_pad=65, auto_scale=True):
    ...
    # ---> THIS IS THE FIX: ONLY ALLOCATE IF NOT ALREADY SET <---
...
def handle_aosp_artifacts_and_build():
    ...
    dynamic_compile_threads = max(3, int(total_cores * 0.70))
...
def print_launch_banner(dash_port=None):
    """Renders left-aligned orchestration banner with robust dashboard port fallback, edge-to-edge gray dividers, and proper top padding."""
    import os
    import sys
    ...
    def make_row(left_text, right_text, left_colored=None, right_colored=None):
        ...
        # Removed left_margin calculation to snap directly to the left edge
        return l_disp + l_pad + (" " * gap) + r_disp
    ...
    # Extended the gray bar to touch the absolute left edge of the terminal
    gray_bar = "\033[90m" + "─" * width + "\033[0m"
    banner_lines.append(gray_bar)

    flags_colored_text = f"Active Flags     : \033[35m{flags_text}\033[0m"
    # Removed padding to snap to left edge
    banner_lines.append(flags_colored_text)
    ...
    # Removed padding to snap to left edge
    banner_lines.append(flags_color)
...
if __name__ == "__main__":
    ...
    try:
        ...
        # ---> ADD THIS BLOCK TO PRE-ALLOCATE THE PORT <---
        import socket
        if not DASHBOARD_PORT:
            with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:
                s.bind(("127.0.0.1", 0))
            ...
            try:
                ...
                pass
        # -------------------------------------------------
        print_launch_banner(DASHBOARD_PORT)
        sys.argv = [arg for arg in sys.argv if arg not in ("stream", "banner")]

        # Launch the UI Dashboard immediately to view logs right away
        if "console" not in sys.argv:
            print("[INFO] Applying preliminary Security & Firewall rules to ensure Dashboard connectivity...")
            configure_host_firewall_for_webrtc()
            setup_apparmor()  # <--- ADD THIS: Patches AppArmor BEFORE the browser launches
